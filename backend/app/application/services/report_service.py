...
class ReportDataAssembler:
    ...
    async def assemble(
        ...
        recs = self._build_recommendations(ai_recommendations or [], vulns)
        ...
        return ReportData(
            ...
            or self._default_summary(org_name, health_score, dist, len(assets)),
    ...
    async def _fetch_trend(self, org_id: str, days: int) -> list[RiskTrendPoint]:
        """
    ...
    @staticmethod
    def _asset_risk(critical: int, high: int, medium: int, low: int, criticality: str) -> float:
    ...
    def _build_recommendations(
        ...
        if not recs:
            critical_vulns = [
                v for v in vulns if v.severity == SeverityLevel.CRITICAL and v.status == "open"
            ][:3]
    ...
    @staticmethod
    def _default_summary(org_name: str, score: int, dist: SeverityDistribution, assets: int) -> str:
        return (
    ...
    def _map_to_compliance_frameworks(
        ...
        pci_controls = [
            ComplianceControl(
                ...
                if any(v.severity in ("critical", "high") for v in vulns if hasattr(v, "severity"))
                else "compliant",
                related_vulns=tuple(
                    v.cve_id for v in vulns[:3] if hasattr(v, "cve_id") and v.cve_id
                ),
...
class ReportService:
    ...
    async def generate(
        ...
        # Step 3: render
        content, filename = await self._render(data, report_format, charts_svg, charts_png)
    ...
    async def _generate_charts(self, data: ReportData) -> tuple[dict[str, str], dict[str, bytes]]:
        """
        ...
        charts_svg = {
            k: (v if isinstance(v, str) else "") for k, v in zip(svg_tasks.keys(), svg_results)
        }

        # Generate PNG versions (for DOCX)
        png_tasks = {
            "health_gauge": loop.run_in_executor(
                None, lambda: gen.health_gauge(data.health_score, as_png=True)
            ),
            "severity_donut": loop.run_in_executor(
                None, lambda: gen.severity_donut(dist, as_png=True)
            ),
            "risk_trend": loop.run_in_executor(
                None, lambda: gen.risk_trend(trend_dicts, as_png=True)
            ),
            "asset_risk": loop.run_in_executor(
                None, lambda: gen.asset_risk_bars(asset_dicts, as_png=True)
            ),
            "asset_stacked": loop.run_in_executor(
                None, lambda: gen.severity_by_asset_stacked(asset_dicts, as_png=True)
            ),
            "compliance": loop.run_in_executor(
                None, lambda: gen.compliance_bars(fw_dicts, as_png=True)
            ),
        }
        png_results = await asyncio.gather(*png_tasks.values(), return_exceptions=True)
        charts_png = {
            k: (v if isinstance(v, bytes) else b"") for k, v in zip(png_tasks.keys(), png_results)
        }
