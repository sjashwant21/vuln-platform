...
class DocxRenderer:
    ...
    async def render(
        ...
        result = await loop.run_in_executor(None, self._build_docx, report_data, charts_png)
        logger.info(
            "docx_rendered",
    ...
    def _build_docx(self, data: ReportData, charts: dict[str, bytes]) -> bytes:
        ...
        p = doc.add_paragraph()
        p.add_run(
            "Executive Summary  ·  Risk Metrics  ·  Asset Inventory  ·  "
            "Vulnerability Details  ·  Risk Trend  ·  Recommendations"
        ).italic = True
        doc.add_paragraph()
    ...
    def _add_cover(self, doc: Any, data: ReportData) -> None:
        ...
        meta = [
            ...
            (
                "Period",
                f"{data.period_start.strftime('%b %d')} – {data.period_end.strftime('%b %d, %Y')}",
            ),
    ...
    def _add_executive_section(self, doc: Any, data: ReportData, charts: dict) -> None:
        ...
        if "health_gauge" in charts:
            self._add_png(
                doc, charts["health_gauge"], width=Inches(3.5), caption="Security Health Score"
            )
    ...
    def _add_asset_section(self, doc: Any, data: ReportData, charts: dict) -> None:
        ...
        if "asset_risk" in charts:
            self._add_png(doc, charts["asset_risk"], width=Inches(6), caption="Asset Risk Scores")
        ...
        for asset in data.assets:
            ...
            self._set_cell_text(
                row.cells[5],
                str(asset.vuln_critical),
                color="critical" if asset.vuln_critical > 0 else "text",
            )
            self._set_cell_text(
                row.cells[6], str(asset.vuln_high), color="high" if asset.vuln_high > 0 else "text"
            )
    ...
    def _add_vuln_section(self, doc: Any, data: ReportData, charts: dict) -> None:
        ...
        if "asset_stacked" in charts:
            self._add_png(
                doc, charts["asset_stacked"], width=Inches(6), caption="Vulnerabilities by Asset"
            )
        ...
        for severity in ["critical", "high", "medium", "low"]:
            ...
            self._add_heading(
                doc, f"{severity.title()} Vulnerabilities ({len(sev_vulns)})", level=2
            )
            ...
            for v in sev_vulns:
                ...
                self._set_cell_text(
                    row.cells[2], f"{v.cvss_score:.1f}" if v.cvss_score else "?", color=severity
                )
                self._set_cell_text(row.cells[3], v.asset_name[:18])
                self._set_cell_text(
                    row.cells[4], str(v.age_days), color="critical" if v.age_days > 30 else "text"
                )
                self._set_cell_text(
                    row.cells[5],
                    "⚠ YES" if v.has_exploit else "No",
    ...
    def _add_compliance_section(self, doc: Any, data: ReportData, charts: dict) -> None:
        ...
        if "compliance" in charts:
            self._add_png(
                doc, charts["compliance"], width=Inches(5.5), caption="Compliance by Framework"
            )
        ...
        for fw in data.compliance:
            ...
            for ctrl in fw.controls:
                ...
                self._set_cell_text(
                    row.cells[2], ctrl.status.replace("_", " ").title(), color=color
                )
    ...
    def _add_trend_section(self, doc: Any, data: ReportData, charts: dict) -> None:
        ...
        if "risk_trend" in charts:
            self._add_png(
                doc, charts["risk_trend"], width=Inches(6.5), caption="Risk Trend (Last 30 Days)"
            )
        ...
        for p in data.risk_trend:
            ...
            self._set_cell_text(
                row.cells[1], str(p.critical), color="critical" if p.critical > 0 else "text"
            )
